...
*/
function generateInviteToken( is_active, team_id, is_sub) {
    const payload = { team_id: team_id, is_sub: is_sub, is_active: is_active }
...
async function requireInvite(req, res, next){
    try{
        ...
        if(invite != null && invite.status == "active"){
            ...
            req.body.is_active = payload.is_active
            req.body.team_id = payload.team_id
            req.body.is_sub = payload.is_sub
            next()
        } else {
            res.status(401).send({
